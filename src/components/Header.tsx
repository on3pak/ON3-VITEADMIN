...
const viewBreadcrumb: Record<DashboardViewType, { title: string; parent?: string; section?: string }> = {
  ...
  MACHINERY_CRUD: { title: 'Maquinaria', parent: 'Administración' },
  MACHINERY_DASHBOARD: { title: 'Maquinaria', parent: 'Dashboard' },
  WORK_REPORT: { title: 'Parte de Trabajo', parent: 'Apps' },
...
export const Header: React.FC<HeaderProps> = ({ currentView, setCurrentView, sidebarOpen, setSidebarOpen, serviceReportTab }) => {
  ...
  return (
    <>
      ...
      <header className="flex lg:hidden items-center fixed z-40 top-0 start-0 end-0 shrink-0 bg-[#f8f9fc] dark:bg-[#07182E] h-16 border-b border-app-border">
