...
export const JwtTestsView: React.FC = () => {
  const [isRunningAll, setIsRunningAll] = useState(false);
  const [testSuite, setTestSuite] = useState<MockTest[]>([
    {
      id: 'T-03',
      name: 'JWT: Validación de Expiración y Emisor',
      category: 'JWT',
      description: 'Comprueba el algoritmo de parseo criptográfico de expiración del token e issuer corporativo.',
      status: 'IDLE',
      assertions: [
    ...
    <div className="space-y-6">
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-bold text-app-text">Tests de JWT</h3>
            <p className="text-xs text-app-text-secondary mt-1">Validación de tokens, expiración y firma criptográfica</p>
          </div>
          <button
            onClick={runAllTests}
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] bg-purple-50 text-purple-700 px-1.5 py-0.2 rounded font-extrabold uppercase font-mono">
                    JWT
                  </span>
                </div>
