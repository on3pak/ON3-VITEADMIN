...
const statusBadgeStyles: Record<string, string> = {
  ...
  'ms-4': 'bg-slate-100 text-slate-700 border-slate-200',
};

const getStatusBadge = (id: string) => (
  <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${statusBadgeStyles[id] || 'bg-slate-50 text-slate-700'}`}>
    {statusNameMap[id] || id}
  </span>
);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-semibold text-slate-800">Por Subtipo</h3>
              <p className="text-xs text-slate-500">Distribución por tipo</p>
            </div>
              ...
              return (
                <div key={st.id}>
                  <div className="flex items-center justify-between text-sm mb-1">
                    ...
                    <span className="text-slate-500 text-xs">{st.count} ({pct.toFixed(0)}%)</span>
                  </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-semibold text-slate-800">Por Estado</h3>
              <p className="text-xs text-slate-500">Situación actual</p>
            </div>
            <TrendingUp className="h-5 w-5 text-slate-400" />
          </div>
          <div className="grid grid-cols-3 gap-3">
            {statusStats.map((s) => (
              <div key={s.id} className="text-center p-3 bg-slate-50 rounded-xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-semibold text-slate-800">Por Ciudad</h3>
            ...
            {cityStats.map((c) => (
              <div key={c.id} className="flex items-center justify-between p-2.5 bg-slate-50 rounded-lg">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-slate-800 w-6 text-right">{c.count}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-semibold text-slate-800">Items Recientes</h3>
              <p className="text-xs text-slate-500">Últimas altas</p>
            </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-200">
                  <th className="text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wide py-2 px-3">Elemento</th>
                  <th className="text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wide py-2 px-3">Cant.</th>
                  <th className="text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wide py-2 px-3">Estado</th>
                  <th className="text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wide py-2 px-3">Fecha</th>
                </tr>
              </thead>
              <tbody>
                {recent.map((item) => (
                  <tr key={item.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                    <td className="py-2.5 px-3">
                      ...
                      <p className="text-[10px] text-slate-400 font-mono">{stNameMap[item.subtype_id] || item.subtype_id} • {wcNameMap[item.work_center_id] || ''}</p>
                    </td>
                    <td className="py-2.5 px-3 text-xs font-bold text-slate-800">{item.quantity}</td>
                    <td className="py-2.5 px-3">{getStatusBadge(item.status_id)}</td>
                    <td className="py-2.5 px-3 text-[10px] text-slate-500">{formatDate(item.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
