...
const STATUS_COLORS: Record<string, string> = {
  ...
  'ms-2': 'text-amber-600 bg-amber-50 border-amber-200 dark:text-amber-300 dark:bg-amber-900/20 dark:border-amber-800',
  'ms-3': 'text-rose-600 bg-rose-50 border-rose-200 dark:text-rose-300 dark:bg-rose-900/20 dark:border-rose-800',
  'ms-4': 'text-app-text-secondary bg-app-bg border-app-border dark:bg-app-border',
};
                ...
                <tbody className="divide-y divide-app-border text-app-text text-sm">
                  {paginatedItems.length === 0 ? (
                    ...
                    paginatedItems.map((item) => {
                      ...
                      const statusClass = STATUS_COLORS[item.status_id] || 'text-app-text-secondary bg-app-bg border-app-border';
                      return (
                        <tr key={item.id} className="hover:bg-app-bg/70 transition-colors">
                          <td className="py-3.5 px-6">
                            <div className="flex items-center gap-3">
                              <div className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0 bg-cyan-100 text-cyan-600 border border-cyan-200 dark:bg-cyan-900/30 dark:text-cyan-300 dark:border-cyan-800">
